...
# Load the dataset
tsne_data = pd.read_csv("data/train_org1.csv")
tsne_data.rename(columns = {'Default':'default'}, inplace=True)
...
# Load the dataset
df = pd.read_csv("data/train_org1.csv")
dfp=pd.read_csv("data/test_org1.csv")
...
for feature in features:
    ...
    mean2,std2=dfp[feature].mean(), dfp[feature].std()
    dfp.loc[:, feature] = (dfp[feature] - mean2) / std2
...
# layer 1
W1 = tf.Variable(tf.truncated_normal([input_nodes, hidden_nodes1], stddev = 0.2))
b1 = tf.Variable(tf.zeros([hidden_nodes1]))
y1 = tf.nn.sigmoid(tf.matmul(x, W1) + b1)

# layer 2
W2 = tf.Variable(tf.truncated_normal([hidden_nodes1, hidden_nodes2], stddev = 0.2))
b2 = tf.Variable(tf.zeros([hidden_nodes2]))
y2 = tf.nn.sigmoid(tf.matmul(y1, W2) + b2)

# layer 3
W3 = tf.Variable(tf.truncated_normal([hidden_nodes2, hidden_nodes3], stddev = 0.2)) 
b3 = tf.Variable(tf.zeros([hidden_nodes3]))
y3 = tf.nn.sigmoid(tf.matmul(y2, W3) + b3)
y3 = tf.nn.dropout(y3, pkeep)

# layer 4
W4 = tf.Variable(tf.truncated_normal([hidden_nodes3, 2], stddev = 0.2)) 
...
# Parameters
training_epochs = 80 # These proved to be enough to let the network learn
    ...
    for epoch in range(training_epochs): 
        ...
        if (epoch) % display_step == 0:
            ...
            valid_accuracy, valid_newCost = sess.run([accuracy, cost], feed_dict={x: inputX_valid, 
                                                                                  y_: inputY_valid,
                                                                                  pkeep: 1})
            train_y_pred,train_y_truth=sess.run([y_pred,y_truth],feed_dict={x: inputX, 
                                                                            y_: inputY,
                                                                            pkeep: training_dropout})
            valid_y_pred,valid_y_truth=sess.run([y_pred,y_truth],feed_dict={x: inputX_valid, 
                                                                                  y_: inputY_valid,
                                                                                  pkeep: 1})

            print(classification_report(train_y_truth,train_y_pred))
            print(classification_report(valid_y_truth,valid_y_pred))

            pred_y=sess.run([y_pred],feed_dict={x:inputX_pred,pkeep:1})
            result=pd.DataFrame(pred_y)
            result.to_csv('result_mttf.csv')
